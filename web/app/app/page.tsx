...
export default function DealSpine() {
  ...
  const { data: view } = useQuery({ queryKey: qk.facility(role), queryFn: () => fetchFacility(role) });
  ...
  return (
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", color: INK, background: "#fff", overflow: "hidden" }}>
      <TopBar view={view} role={role} onRole={switchRole} onInspect={() => setInspector((v) => !v)} inspecting={inspector} />
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "312px 1fr 372px", overflow: "hidden" }}>
        ...
        <div style={{ overflow: "auto", background: "#fff" }}>
          <div style={{ borderBottom: `2px solid ${INK}`, padding: "22px 30px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "#fff", zIndex: 2 }}>
            ...
            <div className="font-mono" style={{ fontSize: 11, color: "#666", letterSpacing: ".06em", textAlign: "right", maxWidth: 240 }}>{meta.note}</div>
          </div>
        ...
        <CopilotRail copilot={copilot} view={view} onAuthorize={onAuthorize} pending={mutation.isPending} />
      </div>
    </div>
  );
}
...
function TopBar({ view, role, onRole, onInspect, inspecting }: { view: FacilityView; role: Role; onRole: (r: Role) => void; onInspect: () => void; inspecting: boolean }) {
  return (
    <div style={{ height: 60, borderBottom: `2px solid ${INK}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flex: "0 0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <span className="font-mono" style={{ fontSize: 10, letterSpacing: ".12em", textTransform: "uppercase", color: "#999" }}>View as</span>
        ...
        <button
          onClick={onInspect}
          className="font-mono"
          style={{ display: "flex", alignItems: "center", gap: 8, border: `2px solid ${inspecting ? MINT : "#e4e4e0"}`, background: inspecting ? MINT : "#fff", padding: "8px 12px", fontSize: 11, fontWeight: 700, letterSpacing: ".06em", textTransform: "uppercase", cursor: "pointer", color: INK }}
        >
          What others see
        </button>
      </div>
...
function OriginationStage({ view }: StageProps) {
...
function LoanTape({ view }: { view: FacilityView }) {
  ...
  return (
    <div style={{ border: `2px solid ${INK}`, background: "#fff" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 18px", background: PAPER, borderBottom: `2px solid ${INK}` }}>
...
function SyndicateTiles({ view }: { view: FacilityView }) {
  const f = view.facility;
  const p = view.position;
...
function TwoLegCard({
  tag, both, leg1Label, leg1Value, leg2Label, leg2Value, cta, note, onClick, pending, locked, blocked,
}: {
  tag: string; both: string; leg1Label: string; leg1Value: string; leg2Label: string; leg2Value: string;
  cta: string; note: string; onClick: () => void; pending: boolean; locked?: boolean; blocked?: boolean;
}) {
    ...
    <div style={{ border: `2px solid ${INK}`, background: INK, color: PAPER, padding: 26 }}>
      ...
      <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "1fr 56px 1fr", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 44, height: 44, border: `2px solid ${blocked ? RED : MINT}`, display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
...
function DrawdownStage({ view, copilot, drawAmount, setDrawAmount, onSettle, pending }: StageProps) {
...
function CovenantImpactBar({ impact }: { impact: { key: string; projected: number; threshold: number; breaches: boolean } }) {
  ...
  return (
    <div style={{ border: `2px solid ${b ? RED : INK}`, background: b ? "#fbeae8" : "#fff", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
...
function InterestStage({ view, onSettle, pending }: StageProps) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {accrued > 0 ? (
        <TwoLegCard
          ...
          onClick={() => onSettle("interest")} pending={pending} locked={!view.canSettle}
        />
      ) : (
        <div style={{ border: `2px solid ${INK}`, background: INK, color: PAPER, padding: 26 }}>
...
function RepaymentStage({ view, onSettle, pending }: StageProps) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <TwoLegCard
        ...
        onClick={() => onSettle("repayment", { amount: facilityAmort })} pending={pending} locked={!view.canSettle}
      />
...
function MaturityStage({ view }: StageProps) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
        <Metric bg={MINT} label="Projected payoff" value={money(view.position.commitment)} sub="full principal returned" />
        <Metric bg="#fff" label="Projected IRR" value="11.8%" sub="to the slice" subMint />
      ...
      <div style={{ border: `2px solid ${INK}`, background: INK, color: PAPER, padding: 26 }}>
        ...
        <div style={{ marginTop: 12, fontWeight: 700, fontSize: 32, letterSpacing: "-.03em", lineHeight: 1.1 }}>
          At current trajectory, this facility retires fully at maturity — every leg along the way settled atomically, each
...
function CopilotRail({ copilot, view, onAuthorize, pending }: { copilot: CopilotProposal | undefined; view: FacilityView; onAuthorize: () => void; pending: boolean }) {
  ...
  const canAuthorize = view.canSettle && !!assessment && assessment.decision === "allow" && assessment.choice !== "none";
  const showGate = !!assessment && assessment.choice !== "none";
  return (
        ...
        {showGate && (
          <div style={{ border: `2px solid ${blocked ? RED : MINT}`, background: blocked ? "#fbeae8" : MINT, padding: "16px 18px" }}>
            ...
            <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
              ...
              <button className="font-mono" style={{ background: blocked ? "#fff" : MINT, border: `2px solid ${INK}`, color: INK, padding: "10px 18px", fontSize: 11, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", cursor: "pointer" }}>
